...
class SparkScalaRunnerProvider extends RunnerSetupProvider {
  ...
  override def getCommand(jobId: String, actionData: ActionData, env: String, executorId: String, callbackAddress: String): String = conf.mode match {
    case "mesos" =>
      s"add-apt-repository -y ppa:openjdk-r/ppa && apt-get update && apt-get -y install libc6-i386 openjdk-8-jre && export MESOS_NATIVE_JAVA_LIBRARY=/opt/mesosphere/libmesos-bundle/lib/libmesos.so && env && pwd &&" +
      s"env AMA_NODE=${sys.env("AMA_NODE")} env MESOS_NATIVE_JAVA_LIBRARY=/opt/mesosphere/libmesos-bundle/lib/libmesos.so env SPARK_EXECUTOR_URI=http://${sys.env("AMA_NODE")}:${conf.Webserver.Port}/dist/spark-${conf.Webserver.sparkVersion}.tgz " +
