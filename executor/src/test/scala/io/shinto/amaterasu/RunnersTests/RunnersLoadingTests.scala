...
class RunnersLoadingTests extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  override protected def beforeAll(): Unit = {
    ...
    env.master = "local[*]"
    factory = ProvidersFactory(ExecData(env, Dependencies(ListBuffer.empty[Repo], List.empty[Artifact]), Map("spark" -> Map.empty[String, Any])), "test", new ByteArrayOutputStream(), new TestNotifier(), "test")
    super.beforeAll()
  }
